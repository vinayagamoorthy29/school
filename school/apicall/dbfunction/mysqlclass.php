...
class MySQL
{
	...
	private $db_user    = "root";     // user namedigispel_website
	private $db_pass    = "";        // passwordcAt8wNG
	private $db_dbname  = "school_db";  // database name
	...
	*/
	...
	static public function SQLValue($value, $datatype = self::SQLVALUE_TEXT) {
		...
		switch (strtolower(trim($datatype))) {
			...
			case "time":
				if (self::IsDate($value)) {
					$return_value = "'" . date('H:i', strtotime($value)) . "'";
				} else {
					$return_value = "NULL";
				}
				break;
